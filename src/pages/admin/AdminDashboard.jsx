...
import { LayoutGrid, Package, ShoppingBag, Users, LogOut, Globe, ClipboardList, Image } from 'lucide-react';
...
import AdminCustomRequests from './AdminCustomRequests.jsx';
import AdminBanner         from './AdminBanner.jsx';        // ← NEW

const TABS = [
  { key: 'orders',     icon: ShoppingBag   },
  { key: 'lists',      icon: ClipboardList },
  { key: 'banner',     icon: Image     },  // ← NEW
...
export default function AdminDashboard() {
  ...
  const handleLogout = () => {
    const msg = lang === 'si' ? 'ඔබට සැබවින්ම පිටවීමට අවශ්‍යද?' : 'Are you sure you want to logout?';
    if (window.confirm(msg)) { logout(); navigate('/login'); }
  };

  const tabLabel = {
    orders:     t('manageOrders'),
    lists:      lang === 'si' ? 'බඩු ලැයිස්තු'  : 'Item Lists',
    banner:     lang === 'si' ? 'බැනරය'          : 'Banner',     // ← NEW
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-gray-800 text-white px-4 h-16 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-sm sm:hidden text-orange-300">{tabLabelShort[tab]}</span>
        </div>
